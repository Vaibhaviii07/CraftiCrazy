...
export default function DiwaliHamperPage() {
  const [items, setItems] = useState<DiwaliHamper[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/diwali"); // API endpoint
        const apiData = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          apiData.map((item: any) => ({
            id: String(item._id || item.id || ""),
            name: item.name || "Diwali Hamper",
            price: item.price || 0,
  ...
  }, [items, highlight, selectedCategories]);
            ...
            <ul className="space-y-2">
              {highlightOptions.map(opt => (
                <li
                  ...
                  className={`text-sm cursor-pointer ${highlight === opt ? "text-[#F7B77A] font-semibold" : "text-gray-700"}`}
                >
                  {opt}
                </li>
              ))}
            ...
            <AnimatePresence>
              {loading
                ...
                : sortedItems.map(item => (
                    ...
                    >
                      <Link to={`/diwalidetail/${item.id}`} className="w-full max-w-[330px] flex flex-col">
                        ...
                        <div className="mt-2 sm:mt-3 text-center px-1 sm:px-0">
                          ...
                          <div className="mt-1 sm:mt-2 flex justify-center gap-1 sm:gap-2 items-baseline">
                            <span className="text-lg sm:text-2xl text-[#F7B77A] font-cinzel">₹{item.price}</span>
                            {item.discount && (
