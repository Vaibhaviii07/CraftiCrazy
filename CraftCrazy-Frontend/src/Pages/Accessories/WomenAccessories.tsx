// src/Pages/Accessories/WomenAccessoriesPage.tsx
...
export default function WomenAccessoriesPage() {
    ...
    const fetchAccessories = async () => {
      try {
        const res = await axios.get(
          "http://localhost:8000/api/products/womenaccessories"
        );
  ...
  const categories = [...new Set(accessories.map((i) => i.category || "Others"))];
  ...
  const filteredItems = useMemo(() => {
    return accessories.filter((item) => {
      const categoryMatch =
        ...
        selectedCategories.includes(item.category || "Others");
