...
export default function BraceletPage() {
  ...
  // Fetch bracelets from API
  useEffect(() => {
    const fetchBracelets = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/bracelets");
        const apiData = res.data?.allProducts || [];
        setBracelets(
          apiData.map((item: any) => ({
            id: String(item._id || item.id || ""), // ensure string
            name: item.name || "Bracelet",
            price: item.price || 0,
  ...
  const categories = [...new Set(bracelets.map(item => item.category || "Others"))]; // always string
            ...
            <AnimatePresence>
              {loading
                ...
                : sortedItems.map(item => (
                    ...
                    >
                      <Link
                        to={`/braceletdetail/${item.id}`} // always string
