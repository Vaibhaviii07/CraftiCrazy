...
export default function VarmalaPreservationPage() {
  const [items, setItems] = useState<VarmalaItem[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/varmala-preservation");
        const data = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          data.map((item: any) => ({
            id: String(item._id || item.id),
            name: item.name || "Varmala Preservation",
            price: item.price || 0,
