// src/Pages/Engagement/EngagementTrayPage.tsx
import React, { useState, useEffect, useMemo } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Link } from "react-router-dom";
import axios from "axios";

interface EngagementTray {
  id: string;
  name: string;
  price: number;
  discount?: number;
...
export default function EngagementTrayPage() {
  const [items, setItems] = useState<EngagementTray[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/engagement-tray");
        const data = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          data.map((item: any) => ({
            id: String(item._id || item.id),
            name: item.name || "Engagement Tray",
            price: item.price || 0,
