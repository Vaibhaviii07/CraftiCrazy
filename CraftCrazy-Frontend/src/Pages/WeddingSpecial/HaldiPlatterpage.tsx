// src/Pages/Haldi/HaldiPlatterPage.tsx
import React, { useState, useEffect, useMemo } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Link } from "react-router-dom";
import axios from "axios";

interface HaldiPlatter {
  id: string;
  name: string;
  price: number;
  discount?: number;
...
export default function HaldiPlatterPage() {
  const [items, setItems] = useState<HaldiPlatter[]>([]);
  ...
  // Fetch platters from backend
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/haldi-platter");
        const data = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          data.map((item: any) => ({
            id: String(item._id || item.id),
            name: item.name || "Haldi Platter",
            price: item.price || 0,
