...
// Full CorporateHamper type
interface CorporateHamper {
  id: string;
  name: string;
  price: number;
  discount?: number;
...
export default function CorporateHamperPage() {
  const [hampers, setHampers] = useState<CorporateHamper[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");

  // Fetch corporate hampers from API
  useEffect(() => {
    const fetchHampers = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/corporate"); // update API
        ...
        setHampers(
          apiData.map((item: any) => ({
            ...
            name: item.name || "Corporate Hamper",
            price: item.price || 0,
  ...
  const categories = useMemo(
    () => [...new Set(hampers.map((i) => i.category || "Others"))],
    [hampers]
  );
                      ...
                      >
                        ...
                        <div className="mt-2 sm:mt-3 text-center px-1 sm:px-0">
                          ...
                          <div className="mt-1 sm:mt-2 flex justify-center gap-1 sm:gap-2 items-baseline">
                            ...
                            {item.discount && (
                              <span className="line-through text-gray-400 text-sm sm:text-lg ml-2">₹{Math.round(item.price + (item.price * item.discount) / 100)}</span>
                            )}
                          </div>
                        </div>
                      </Link>
