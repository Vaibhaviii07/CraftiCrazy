...
export default function ResinFramePage() {
  const [items, setItems] = useState<ResinFrame[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/resinframe"); // API endpoint
            ...
            <AnimatePresence>
              {loading
                ...
                : sortedItems.map(item => (
                  ...
                  >
                    <Link to={`/Framedetail/${item.id}`} className="w-full max-w-[320px] flex flex-col">
