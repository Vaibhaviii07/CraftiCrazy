...
export default function WoodenFramePage() {
  const [items, setItems] = useState<WoodenFrame[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/woodenframe");
        const apiData = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          apiData.map((item: any) => ({
            id: String(item._id || item.id || ""),
            name: item.name || "Wooden Frame",
            price: item.price || 0,
            ...
            <AnimatePresence>
              {loading
                ...
                : sortedItems.map(item => (
                    ...
                    >
                      <Link to={`/woodendetail/${item.id}`} className="w-full max-w-[320px] flex flex-col">
