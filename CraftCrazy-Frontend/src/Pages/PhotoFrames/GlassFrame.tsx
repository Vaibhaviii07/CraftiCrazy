...
export default function GlassFramePage() {
  const [items, setItems] = useState<GlassFrame[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/glassframe"); // your API endpoint
        const apiData = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          apiData.map((item: any) => ({
            id: String(item._id || item.id || ""),
            name: item.name || "Glass Frame",
            price: item.price || 0,
    ...
    <section className="min-h-screen">
      ...
      <div className="text-center mt-10 mb-8">
        ...
        <p className="mt-3 text-gray-600 text-base italic max-w-sm mx-auto">
          Stylish and elegant glass frames for your home or office.
        </p>
      </div>
            ...
            <AnimatePresence>
              {loading
                ...
                : sortedItems.map(item => (
                  ...
                  >
                    <Link to={`/glassdetail/${item.id}`} className="w-full max-w-[320px] flex flex-col">
