...
export default function ResinCoasterSetPage() {
  const [items, setItems] = useState<ResinCoasterSet[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/resin-coaster-set");
        const apiData = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          apiData.map((item: any) => ({
            id: String(item._id || item.id || ""),
            name: item.name || "Resin Coaster Set",
            price: item.price || 0,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 mt-12 sm:mt-16 grid grid-cols-1 md:grid-cols-5 gap-6 md:gap-8">
            ...
            <ul className="space-y-2">
              {highlightOptions.map(opt => (
                <li
                  ...
                  className={`text-sm cursor-pointer ${highlight === opt ? "text-[#b46029] font-semibold" : "text-gray-700 hover:text-[#b46029]"}`}
                >
                  {opt}
                </li>
              ))}
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8 mb-16">
            <AnimatePresence>
              {loading
                ? Array(6).fill(0).map((_, i) => <div key={i} className="bg-gray-200 rounded-3xl h-[360px] animate-pulse"></div>)
                : sortedItems.map(item => (
                    ...
                    >
                      <Link to={`/caosterdetail/${item.id}`} className="w-full max-w-[300px] sm:max-w-[340px] flex flex-col">
                        <div className="relative w-full h-[260px] sm:h-[320px] lg:h-[360px] rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-transform duration-500 hover:-translate-y-2 sm:hover:-translate-y-3">
                        ...
                        <div className="mt-2 sm:mt-3 text-center">
                          ...
                          <div className="mt-1 sm:mt-2 flex justify-center gap-1 sm:gap-2 items-baseline">
                            ...
                            {item.discount && <span className="line-through text-gray-400 text-sm sm:text-lg ml-2">₹{Math.round(item.price / (1 - (item.discount ?? 0) / 100))}</span>}
                          </div>
                        </div>
                      </Link>
                    </motion.div>
