...
export default function ResinJewelryPage() {
  const [items, setItems] = useState<ResinJewelry[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/resin-jewelry");
        const apiData = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          apiData.map((item: any) => ({
            id: String(item._id || item.id || ""),
            name: item.name || "Resin Jewelry",
            price: item.price || 0,
            ...
            <AnimatePresence>
              {loading
                ...
                : sortedItems.map(item => (
                    ...
                    >
                      <Link to={`/jewelarydetail/${item.id}`} className="w-full max-w-[280px] sm:max-w-[320px] flex flex-col">
