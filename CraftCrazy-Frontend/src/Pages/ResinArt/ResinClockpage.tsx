...
export default function ResinClockPage() {
  const [items, setItems] = useState<ResinClock[]>([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await axios.get("http://localhost:8000/api/products/resin-clock");
        const apiData = Array.isArray(res.data) ? res.data : res.data?.allProducts || [];
        setItems(
          apiData.map((item: any) => ({
            id: String(item._id || item.id || ""),
            name: item.name || "Resin Clock",
            price: item.price || 0,
            ...
            <AnimatePresence>
              {loading
                ...
                : sortedItems.map(item => (
                    ...
                    >
                      <Link to={`/clockdetail/${item.id}`} className="w-full max-w-[320px] flex flex-col">
                        ...
                        <div className="mt-2 sm:mt-3 text-center">
                          ...
                          <div className="mt-1 sm:mt-2 flex justify-center gap-2 items-baseline">
                            ...
                            {item.discount && <span className="line-through text-gray-400 text-sm sm:text-lg">₹{Math.round(item.price / (1 - (item.discount ?? 0) / 100))}</span>}
                          </div>
                        </div>
                      </Link>
                    </motion.div>
