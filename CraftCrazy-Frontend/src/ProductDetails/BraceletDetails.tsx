...
type Bracelet = {
  ...
  /* You used these fields in JSX, so I added them */
  brand?: string;
  seller?: string;
  returnPolicy?: string;
};
...
export default function BraceletDetails() {
  ...
  /* 🔹 Fetch product from backend or static data */
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/bracelets/${id}`);
        ...
        if (data?.product) {
          ...
          const staticProd = staticBracelets.find((p) => p.id === id);
          if (staticProd) {
            setCurrentProduct(staticProd);
            setCurrentVariant(
        ...
        console.log("Backend fetch failed → using static data.");
  ...
  useEffect(() => {
    const fetchRatingData = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/reviews/rating/${id}`);
        const data = await res.json();
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            <div className="flex items-center gap-1">
              ...
              <span className="ml-1 text-gray-600 text-sm">
                ({finalRating.toFixed(1)} | {backendReviewsCount} reviews)
              </span>
            </div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-[#E8D4B7] text-black px-6 py-3 rounded-lg shadow-lg"
          >
            {toast}
          </motion.div>
        )}
