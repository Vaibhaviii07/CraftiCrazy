...
export default function HaldiPlatterDetailPage() {
  ...
  const staticProduct = haldiPlatters.find(p => String(p.id) === id);
  const [currentProduct, setCurrentProduct] = useState<HaldiPlatter | null>(staticProduct ?? null);
  ...
  // Selected variant (guaranteed id)
  ...
  // Reset variant, quantity, and reviews when selectedVariant changes
  useEffect(() => {
