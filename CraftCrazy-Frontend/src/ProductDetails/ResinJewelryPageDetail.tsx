...
import { resinJewelry, ResinJewelry, Variant } from "../Data/ResinJewelryData";
...
export default function ResinJewelryDetailPage() {
  ...
  // Initial static product fallback
  const productFromParams: ResinJewelry | undefined = resinJewelry.find(j => j.id === id);
  const [currentProduct, setCurrentProduct] = useState<ResinJewelry | null>(productFromParams ?? null);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/resinJewelry/${id}`);
