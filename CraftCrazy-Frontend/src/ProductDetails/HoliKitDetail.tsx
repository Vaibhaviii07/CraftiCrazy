...
export default function HoliKitDetail() {
  ...
  const staticProduct = holiKits.find(p => String(p.id) === id);
  const [currentProduct, setCurrentProduct] = useState<HoliKit | null>(staticProduct ?? null);
  ...
  // Selected variant (with guaranteed id)
  ...
  // Reset variant, quantity, and reviews when variant changes
  useEffect(() => {
