...
export default function ResinClockDetailPage() {
  ...
  const productFromParams: ResinClock | undefined = resinClocks.find(p => String(p.id) === id);
  const [currentProduct, setCurrentProduct] = useState<ResinClock | null>(productFromParams ?? null);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        else setCurrentProduct(resinClocks.find(p => String(p.id) === id) ?? null);
      } catch (err) {
        setCurrentProduct(resinClocks.find(p => String(p.id) === id) ?? null);
      }
    };
