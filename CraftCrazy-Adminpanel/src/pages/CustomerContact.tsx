...
const CustomerContact: React.FC = () => {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    console.log("Contact Api is running...");
      const res = await axios.get(`http://localhost:8000/api/contact/all`, { withCredentials: true });
 ...
 useEffect(() => {
  ...
  const markResolved = async (id: string) => {
    try {
      setProcessingId(id);
      await axios.patch(`http://localhost:8000/api/contact/update-status/${id}`, { status: "resolved" });
