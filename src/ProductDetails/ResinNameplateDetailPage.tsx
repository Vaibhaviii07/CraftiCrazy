...
export default function ResinNameplateDetailPage() {
  ...
  const productFromParams: ResinNameplate | undefined = resinNameplates.find(
    (p) => String(p.id) === id
  );
  ...
  const [currentProduct] = useState<ResinNameplate>(productFromParams);
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            <div className="flex items-center gap-1">
              {Array.from({ length: Math.floor(currentProduct.rating || 0) }).map(
                (_, i: number) => <Star key={i} className="w-5 h-5 text-yellow-400" />
              )}
            </div>
