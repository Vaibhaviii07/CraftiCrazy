...
export default function ResinNameplateDetailPage() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem(`reviews_${currentProduct.id}`);
    const parsed: Review[] = stored ? JSON.parse(stored) : [];
    setReviews(parsed);
    setAverageRating(parsed.length ? Math.round(parsed.reduce((acc, r) => acc + r.rating, 0) / parsed.length) : 0);

    const handleNewReview = (e: any) => {
      if (e.detail === currentProduct.id) {
        const storedNew = localStorage.getItem(`reviews_${currentProduct.id}`);
        const parsedNew: Review[] = storedNew ? JSON.parse(storedNew) : [];
        setReviews(parsedNew);
        setAverageRating(parsedNew.length ? Math.round(parsedNew.reduce((acc, r) => acc + r.rating, 0) / parsedNew.length) : 0);
      }
    };
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="flex-1 relative">
          {!imgLoaded && <div className="w-full h-[400px] sm:h-[500px] rounded-3xl bg-gray-200 animate-pulse"></div>}
          {currentVariant && (
            <motion.img
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-5">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            <span className="text-2xl sm:text-3xl font-semibold text-[#C45A36]">₹{currentVariant?.price ?? 0}</span>
            {currentVariant?.discount && (
              <span className="line-through text-gray-400 text-lg ml-2">₹{currentProduct.price ?? 0}</span>
            )}
          </div>
