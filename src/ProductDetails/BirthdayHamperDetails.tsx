...
export default function BirthdayHamperDetails() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/products/birthday/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: BirthdayHamper = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, falling back to static data:", error);
        // fallback to static data
        const staticProduct = birthdayHampers.find(p => p.id === id) ?? null;
  ...
  // Reset variant & quantity on product change
  useEffect(() => {
