...
export default function BirthdayHamperDetails() {
  ...
  const productFromParams = birthdayHampers.find((p) => String(p.id) === id);
  if (!productFromParams) {
    return <p className="text-center mt-10 text-lg">Product not found</p>;
  }

  const [currentProduct, setCurrentProduct] = useState(productFromParams);

  const [selectedVariant, setSelectedVariant] = useState<Variant>({
  image: productFromParams.variants?.[0]?.image ?? productFromParams.image ?? "",
  price: productFromParams.variants?.[0]?.price ?? productFromParams.price ?? "",
  discount:
    productFromParams.variants?.[0]?.discount ??
    (productFromParams.discount ?? undefined), 
});
...
useEffect(() => {
  ...
  const similar = birthdayHampers.filter(
  ...
  addToCart({
    ...
    quantity,
    image: selectedVariant.image, // always string
    rating: currentProduct.rating,
    discount: selectedVariant.discount ?? currentProduct.discount ?? undefined, // 👈 ensure no null
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col justify-start">
          ...
          <div className="mt-8 space-y-4">
            ...
            {currentProduct.contents?.length > 0 && (
              <div>
            ...
            {currentProduct.highlight && <p className="text-gray-700 italic">⭐ {currentProduct.highlight}</p>}
            ...
            {currentProduct.occasion?.length > 0 && (
              <p className="text-gray-600">
                <span className="font-semibold">Occasion:</span> {currentProduct.occasion.join(", ")}
              </p>
            )}
          </div>
        </div>
