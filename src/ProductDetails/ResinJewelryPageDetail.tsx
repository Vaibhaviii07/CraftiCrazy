// src/Pages/ResinJewelry/ResinJewelryDetailPage.tsx
import { useParams } from "react-router-dom";
import { useState, useEffect, useMemo } from "react";
import { Variant, ResinJewelry, resinJewelry } from "../Data/ResinJewelryData";
...
export default function ResinJewelryDetailPage() {
  ...
  const [currentProduct, setCurrentProduct] = useState<ResinJewelry | null>(null);
  const [averageRating, setAverageRating] = useState(0);

  // Fetch product dynamically with fallback to static data
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/resinJewelry/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: ResinJewelry = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = resinJewelry.find(p => p.id === id) ?? null;
  ...
  if (!currentProduct) return <p className="text-center mt-20 text-lg text-gray-400">Jewelry not found</p>;
