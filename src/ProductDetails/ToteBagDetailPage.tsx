...
export default function ToteBagDetailPage() {
  ...
  const [currentProduct, setCurrentProduct] = useState<ToteBag | null>(null);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/totebags/${id}`);
        if (!res.ok) throw new Error(`Failed to fetch Tote Bag: ${res.status}`);
        const data: ToteBag = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.warn("⚠️ API error, using fallback data:", error);
        const fallback = toteBags.find((b) => String(b.id) === id) ?? null;
  ...
  useEffect(() => {
    ...
    setAverageRating(
      parsed.length
        ? Math.round(parsed.reduce((acc, r) => acc + r.rating, 0) / parsed.length)
        : 0
    );
    ...
    const handleNewReview = (e: any) => {
      if (e.detail === currentProduct.id) {
        ...
        setAverageRating(
          parsedNew.length
            ? Math.round(
                parsedNew.reduce((acc, r) => acc + r.rating, 0) /
                  parsedNew.length
              )
            : 0
        );
