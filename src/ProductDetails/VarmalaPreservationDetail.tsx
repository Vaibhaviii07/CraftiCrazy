// src/ProductDetails/VarmalaPreservationDetail.tsx
...
export default function VarmalaPreservationDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/varmala/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: VarmalaPreservation = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("API failed, using static fallback:", error);
        const fallback = varmalaPreservations.find(p => String(p.id) === id) ?? null;
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-5">
          ...
          {/* Contents / Customization / Delivery / Specifications */}
