...
import { Variant, EngagementTray, engagementTrays } from "../Data/EngagementTrayData";
...
export default function EngagementTrayDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/engagement-tray/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: EngagementTray = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = engagementTrays.find(p => p.id === id) ?? null;
