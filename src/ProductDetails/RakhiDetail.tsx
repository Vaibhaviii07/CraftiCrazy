...
import { RakhiKit, Variant, rakhiKits } from "../Data/RakhiData";
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const data: RakhiKit = await res.json();
        setCurrentProduct(data);
      } catch (err) {
        console.error("API fetch failed, using fallback static data:", err);
        const fallback = rakhiKits.find(p => String(p.id) === id) ?? null;
