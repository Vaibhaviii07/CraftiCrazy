// src/ProductDetails/ResinJewelryDetailPage.tsx
...
export default function ResinJewelryDetailPage() {
  ...
  const jewelry: ResinJewelry | undefined = resinJewelry.find(
    (j) => j.id === String(id) // Convert string to number
  );
  const { cart, addToCart } = useCart();
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  if (!jewelry) return <p className="text-center mt-20">Jewelry not found!</p>;
  ...
  const product = {
    ...jewelry,
    quantity: 1,
    price: jewelry.price.toString(), // convert to string if CartItem expects string
  };

  const exists = cart.find((c) => c.id === jewelry.id);
  if (!exists) {
    addToCart(product);
    setToastMessage(`${jewelry.name} added to cart ✅`);
  } else {
    setToastMessage(`${jewelry.name} is already in your cart 🛒`);
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div
        ...
        style={{
          backgroundImage: `url(${jewelry.image})`,
      ...
      <div className="max-w-5xl mx-auto px-6 sm:px-10 mt-12 flex flex-col md:flex-row gap-10">
        ...
        <div className="md:w-1/2 flex flex-col gap-4">
          <h2 className="text-3xl font-playfair font-semibold">{jewelry.name}</h2>
          {jewelry.description && (
            <p className="text-gray-600 text-lg">{jewelry.description}</p>
          )}
