...
import { Variant, CorporateHamper, corporateHampers } from "../Data/CorporateData"; 
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const data: CorporateHamper = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = corporateHampers.find(p => p.id === id) ?? null;
