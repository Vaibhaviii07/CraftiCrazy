// src/Pages/GlassFrames/GlassFrameDetails.tsx
...
export default function GlassFrameDetails() {
  ...
  const productFromParams = glassFrames.find((g) => g.id === id);
  if (!productFromParams) {
    return <p className="text-center mt-10 text-lg">Product not found</p>;
  }

  const [currentProduct, setCurrentProduct] = useState<GlassFrame>(productFromParams);
  ...
  const similar = glassFrames.filter(
    (g) => g.category === currentProduct.category && g.id !== currentProduct.id
  );
           ...
           {currentProduct.occasion?.length ? (
              <div className="flex flex-wrap gap-2">
                {currentProduct.occasion.map((o, idx) => (
                  <span key={idx} className="bg-gray-100 text-gray-800 px-3 py-1 rounded-full text-sm semibold">
                    {o}
                  </span>
                ))}
              </div>
