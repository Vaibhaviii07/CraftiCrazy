...
export default function GlassFrameDetailPage() {
  ...
  const productFromParams: GlassFrame | undefined = glassFrames.find(
    (p: GlassFrame) => p.id === id
  );
  ...
  const [currentProduct] = useState<GlassFrame>(productFromParams);
  ...
  const handleAddToCart = () => {
    addToCart({
      ...
      price: `₹${selectedVariant.price}`, // ✅ keep as string for CartContext
      quantity,
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-5">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
            <div className="flex items-center gap-1">
              {Array.from({
                length: Math.floor(currentProduct.rating || 0),
              }).map((_, i: number) => (
