...
import { Variant, GlassFrame, glassFrames } from "../Data/GlassFrameData";
...
export default function GlassFrameDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/glass-frame/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: GlassFrame = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product, using static fallback:", error);
        const fallback = glassFrames.find(p => p.id === id) ?? null;
