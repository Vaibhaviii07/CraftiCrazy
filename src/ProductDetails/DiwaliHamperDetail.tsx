...
import { Variant, DiwaliHamper, diwaliHampers } from "../Data/DiwaliHamperdata";
...
export default function DiwaliHamperDetailPage() {
  ...
  const [currentProduct, setCurrentProduct] = useState<DiwaliHamper | null>(null);
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/diwali-hamper/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: DiwaliHamper = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = diwaliHampers.find(p => p.id === id) ?? null;
