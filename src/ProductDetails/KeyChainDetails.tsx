...
import { KeyChain, Variant, keyChains } from "../Data/KeyChainData";
...
export default function KeyChainDetailPage() {
  ...
  const [currentProduct, setCurrentProduct] = useState<KeyChain | null>(null);
  const [currentVariant, setCurrentVariant] = useState<Variant | null>(null);

  // Reviews state
  const [averageRating, setAverageRating] = useState(0);

  // Fetch product dynamically with fallback
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/keychain/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: KeyChain = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = keyChains.find((p) => p.id === id) ?? null;
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="flex-1 relative">
          {!imgLoaded && <div className="w-full h-[400px] sm:h-[500px] rounded-3xl bg-gray-200 animate-pulse" />}
          {currentVariant && (
            <motion.img
