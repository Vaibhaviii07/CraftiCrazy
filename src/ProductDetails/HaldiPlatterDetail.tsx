// src/Pages/HaldiPlatterDetailPage.tsx
import { useParams } from "react-router-dom";
import { useState, useEffect, useMemo } from "react";
import { HaldiPlatter, haldiPlatters, Variant } from "../Data/HaldiPlatterData";
...
export default function HaldiPlatterDetailPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/haldi/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: HaldiPlatter = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = haldiPlatters.find(p => String(p.id) === id) ?? null;
