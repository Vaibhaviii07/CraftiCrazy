...
import { resinPujaThales, ResinPujaThale, Variant } from "../Data/ResinPujaThaleData";
...
export default function ResinPujaThaleDetailPage() {
  ...
  // ✅ Fetch product dynamically from API (with fallback)
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/resinPujaThales/${id}`);
        if (!res.ok) throw new Error(`API error: ${res.status}`);
        const data: ResinPujaThale = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using fallback:", error);
        const fallback = resinPujaThales.find((p) => String(p.id) === id) ?? null;
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-5">
          ...
          <div className="space-y-2 text-gray-700">
            ...
            {currentProduct.careInstructions && (
              <p><span className="font-semibold">Care:</span> {currentProduct.careInstructions}</p>
            )}
          </div>
