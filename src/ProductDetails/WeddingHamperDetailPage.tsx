...
export default function WeddingHamperDetails() {
  ...
  // Fetch dynamic product data with fallback to static
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/wedding-hampers/${id}`);
        if (!res.ok) throw new Error(`API error ${res.status}`);
        const data: WeddingHamper = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.warn("API failed, using static data:", error);
        const fallback = weddingHampers.find((p) => p.id === id) ?? null;
