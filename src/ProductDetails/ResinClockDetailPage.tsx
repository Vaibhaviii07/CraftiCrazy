// src/Pages/ResinCoaster/ResinClockDetails.tsx
...
export default function ResinClockDetails() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/resinclock/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: ResinClock = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = resinClocks.find(p => p.id === id) ?? null;
