...
import { HoliKit, Variant, holiKits } from "../Data/HoliKitData";
...
export default function HoliKitDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/holi/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: HoliKit = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, using static fallback:", error);
        const fallback = holiKits.find(p => String(p.id) === id) ?? null;
            ...
            <div className="mt-4 flex gap-3 overflow-x-auto py-1 snap-x snap-mandatory">
              {currentProduct.variants.map((v, i) => (
                <motion.div
                  ...
                  className={`relative cursor-pointer border-2 rounded-lg overflow-hidden flex-shrink-0 snap-start ${selectedVariant?.image === v.image ? "border-[#C45A36] ring-2 ring-[#C45A36]" : "border-gray-300"}`}
                  whileHover={{ scale: 1.05 }}
                >
