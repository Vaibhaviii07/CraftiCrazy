...
import { bracelets, Bracelet, Variant } from "../Data/BraceletData";
...
export default function BraceletDetails() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await fetch(`/api/products/bracelet/${id}`);
        if (!res.ok) throw new Error(`API responded with status ${res.status}`);
        const data: Bracelet = await res.json();
        setCurrentProduct(data);
      } catch (error) {
        console.error("Error fetching product from API, falling back to static data:", error);
        const staticProduct = bracelets.find(p => String(p.id) === id) ?? null;
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="flex-1 flex flex-col gap-4 sm:gap-5 ">
          ...
          {/* Additional details */}
          <div className="mb-2 space-y-2 text-gray-700">
