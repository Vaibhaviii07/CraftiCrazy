...
export default function BraceletDetails() {
  ...
  const productFromParams = bracelets.find((b) => String(b.id) === id);
  if (!productFromParams) {
    return <p className="text-center mt-10 text-lg">Product not found</p>;
  }

  const [currentProduct, setCurrentProduct] = useState<Bracelet>(productFromParams);
  ...
  const handleAddToCart = () => {
    addToCart({
      ...
      discount: selectedVariant.discount ?? currentProduct.discount,
  ...
  const similar = bracelets.filter(
    (b) => b.category === currentProduct.category && b.id !== currentProduct.id
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="relative">
          ...
          {currentProduct.variants && currentProduct.variants.length > 1 && (
            <div className="mt-10 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 justify-center">
        ...
        <div className="flex flex-col justify-start">
          ...
          <div className="mt-8 space-y-4">
            ...
            {/* Contents */}
            {currentProduct.contents && currentProduct.contents.length > 0 && (
              <div>
            ...
            {currentProduct.delivery && (
              <div>
                ...
                <p className="text-gray-600">
                  <span className="font-medium">Type:</span> {currentProduct.delivery.type}
                </p>
                <p className="text-gray-600">
                  <span className="font-medium">Availability:</span> {currentProduct.delivery.availability}
                </p>
                <p className="text-gray-600">
                  <span className="font-medium">Estimated:</span> {currentProduct.delivery.estimated}
                </p>
              </div>
            )}

            {/* Customization */}
            {currentProduct.customization && currentProduct.customization.available && (
              <div>
                <h3 className="font-semibold text-gray-800 mb-2">Customization Options:</h3>
                {currentProduct.customization.options && currentProduct.customization.options.length > 0 ? (
                  <ul className="list-disc list-inside text-gray-600 space-y-1">
                    {currentProduct.customization.options.map((option, idx) => (
                      <li key={idx}>{option}</li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-gray-600">Customization is available.</p>
                )}
              </div>
            )}

            {/* Occasion */}
            {currentProduct.occasion && currentProduct.occasion.length > 0 && (
              <div>
                <h3 className="font-semibold text-gray-800 mb-2">Suitable for:</h3>
