...
import { resinFrames as staticFrames, ResinFrame } from "../../Data/ResinFramedata";
import { Link } from "react-router-dom";

export default function ResinPhotoFramesPage() {
  const [frames, setFrames] = useState<ResinFrame[]>(staticFrames);
  ...
  useEffect(() => {
    const fetchFrames = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/resinframes");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: ResinFrame[] = data.map((item: any) => ({
            id: item._id || item.id,
        ...
        console.error("ResinFrames API fetch failed:", err.message);
  ...
  const categories = [...new Set(frames.map(i => i.category))];
  ...
  }, [frames, selectedCategories, highlight]);
  ...
  // Card component to handle lazy-loaded images
  const FrameCard = ({ item }: { item: ResinFrame }) => {
    ...
    return (
      ...
      >
        <Link to={`/photoframedetail/${item.id}`} className="w-full max-w-[280px] sm:max-w-[320px] flex flex-col">
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-12">
              <AnimatePresence>
                {sortedItems.map(item => <FrameCard key={item.id} item={item} />)}
              </AnimatePresence>
            </div>
          )}
        </div>
