...
import { resinPujaThales as staticPujaThales, ResinPujaThale as StaticResinPujaThale } from "../../Data/ResinPujaThaleData";
import { Link } from "react-router-dom";

export default function ResinPujaThalePage() {
  const [pujaThales, setPujaThales] = useState<StaticResinPujaThale[]>(staticPujaThales);
  ...
  // Fetch from backend
  useEffect(() => {
    const fetchPujaThales = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/pujathales");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticResinPujaThale[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setPujaThales(apiMapped);
        } else {
          console.warn("API returned empty, using static fallback.");
        }
      } catch (err: any) {
        console.error("PujaThale API fetch failed:", err.message);
  ...
  const categories = [...new Set(pujaThales.map(i => i.category))];

  // Filtered items
  const filteredItems = useMemo(() => {
    return pujaThales.filter(item => {
      const categoryMatch =
  ...
  }, [pujaThales, selectedCategories, highlight]);
    ...
    <section className="min-h-screen ">
      ...
      <div className="text-center mt-10 mb-8">
        ...
        <p className="mt-3 text-gray-600 text-base italic max-w-sm mx-auto">
          Exquisite resin puja thalis crafted for devotion, beauty, and elegance.
        </p>
      </div>
...
const ProductCard = ({ item }: { item: StaticResinPujaThale }) => {
