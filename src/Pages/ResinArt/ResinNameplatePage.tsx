...
import { resinNameplates as staticNameplates, ResinNameplate as StaticResinNameplate } from "../../Data/ResinNameplateData";
import { Link } from "react-router-dom";

export default function ResinNameplatePage() {
  const [nameplates, setNameplates] = useState<StaticResinNameplate[]>(staticNameplates);
  ...
  useEffect(() => {
    const fetchNameplates = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/resinnameplates");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticResinNameplate[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setNameplates(apiMapped);
        } else {
          console.warn("API returned empty data, using static fallback.");
        }
      } catch (err: any) {
        console.error("ResinNameplate API fetch failed:", err.message);
  ...
  const categories = [...new Set(nameplates.map(i => i.category))];

  // Filter
  const filteredItems = useMemo(() => {
    return nameplates.filter(item => {
  ...
  }, [nameplates, selectedCategories, highlight]);
