// src/Pages/ResinClock/ResinClockPage.tsx
import React, { useState, useMemo, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { Link } from "react-router-dom";
import { ResinClock } from "../../Data/ResinWallClockdata";
import { resinClocks } from "../../Data/ResinWallClockdata"; // fallback data

export default function ResinClockPage() {
  const [items, setItems] = useState<ResinClock[]>(resinClocks); // fallback
  ...
  useEffect(() => {
    const fetchClocks = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/resinclocks");
        ...
        console.error("Resin Clocks API fetch failed:", err.message);
  ...
  const ResinClockCard = ({ item }: { item: ResinClock }) => {
      ...
      <div className="text-center mt-10 mb-8 px-4">
        <h2 className="text-3xl md:text-4xl font-[Playfair_Display] font-bold text-gray-900 relative inline-block">
          ...
          <span className="absolute left-1/2 transform -translate-x-1/2 -bottom-2 w-28 h-1 
            bg-gradient-to-r from-[#b46029] via-[#F7B77A] to-[#b46029] rounded-full animate-pulse"></span>
        </h2>
        <p className="mt-3 text-gray-600 text-base italic max-w-sm mx-auto">
          Elegant resin clocks that brighten your space with charm and creativity.
        </p>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
              <AnimatePresence>
                {sortedItems.map((item) => (
                  <ResinClockCard key={item.id} item={item} />
                ))}
              </AnimatePresence>
            </div>
          )}
