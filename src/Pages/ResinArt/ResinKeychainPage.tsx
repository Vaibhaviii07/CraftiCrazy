...
import { resinKeychains as staticResinKeychains, ResinKeychain as StaticResinKeychain } from "../../Data/ResinKeychainData";
import { Link } from "react-router-dom";

export default function ResinKeychainPage() {
  const [resinKeychains, setResinKeychains] = useState<StaticResinKeychain[]>(staticResinKeychains);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");
  const [loading, setLoading] = useState(true);

  // Fetch Resin Keychains from backend
  useEffect(() => {
    const fetchKeychains = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/resinkeychains");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
         const apiMapped: StaticResinKeychain[] = data.map((item: any) => ({
          id: item._id || item.id,
          sku: item.sku || `SKU-${item._id || item.id}`, // add SKU
          name: item.name,
          ...
          image: item.image,
          inStock: item.inStock ?? true, // add inStock
          highlight: item.highlight || "",
        }));

          setResinKeychains(apiMapped);
        } else {
          console.warn("API returned empty data, using static fallback.");
        }
      } catch (err: any) {
        console.error("ResinKeychain API fetch failed:", err.message);
  ...
  const categories = [...new Set(resinKeychains.map((i) => i.category))];

  // Filtered items
  const filteredItems = useMemo(() => {
    return resinKeychains.filter((item) => {
      const categoryMatch =
  ...
  }, [resinKeychains, selectedCategories, highlight]);
    ...
    <section className="min-h-screen">
      ...
      <div className="text-center mt-10 mb-8">
        <h2 className="text-3xl md:text-4xl font-[Playfair_Display] font-bold text-gray-900 relative inline-block">
          Resin Keychains
          <span className="absolute left-1/2 transform -translate-x-1/2 -bottom-2 w-24 h-1 bg-gradient-to-r from-[#b46029] via-[#F7B77A] to-[#b46029] rounded-full animate-pulse"></span>
        </h2>
        ...
        <div className="md:col-span-4 flex flex-col gap-4">
          ...
          {loading ? (
            <div className="flex justify-center items-center h-64">
              <div className="w-12 h-12 border-4 border-[#b46029] border-t-transparent rounded-full animate-spin"></div>
            </div>
          ) : (
