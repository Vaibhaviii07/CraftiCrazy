...
export default function ResinCoasterSetPage() {
  const [items, setItems] = useState<ResinCoasterSet[]>(resinCoasterSets); // fallback
  ...
  useEffect(() => {
    const fetchCoasters = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/resincoasters");
        ...
        console.error("Resin Coasters API fetch failed:", err.message);
  ...
  const ResinCoasterCard = ({ item }: { item: ResinCoasterSet }) => {
    ...
    <section className="min-h-screen">
      ...
      <div className="text-center mt-10 mb-8 px-4">
        ...
        <p className="mt-3 text-gray-600 text-base italic max-w-sm mx-auto">
          Handcrafted resin coaster sets to preserve your memories in style.
        </p>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
              <AnimatePresence>
                {sortedItems.map((item) => (
                  <ResinCoasterCard key={item.id} item={item} />
                ))}
              </AnimatePresence>
            </div>
          )}
