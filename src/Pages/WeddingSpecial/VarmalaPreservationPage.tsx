...
import { varmalaPreservations as staticVarmala, VarmalaPreservation as StaticVarmala } from "../../Data/VarmalaPreservationdata";
import { Link } from "react-router-dom";

export default function VarmalaPreservationPage() {
  const [varmalas, setVarmalas] = useState<StaticVarmala[]>(staticVarmala);
  ...
  // Fetch from backend
  useEffect(() => {
    const fetchVarmalas = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/varmalas");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticVarmala[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setVarmalas(apiMapped);
        } else {
          console.warn("API returned empty data, using static fallback.");
        }
      } catch (err: any) {
        console.error("Varmala API fetch failed:", err.message);
  ...
  const categories = [...new Set(varmalas.map(i => i.category))];

  // Filtered items
  const filteredItems = useMemo(() => {
    return varmalas.filter(item => {
  ...
  }, [varmalas, selectedCategories, highlight]);
