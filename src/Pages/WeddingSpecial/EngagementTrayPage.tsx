...
import { engagementTrays as staticTrays, EngagementTray as TrayType } from "../../Data/EngagementTrayData";
import { Link } from "react-router-dom";

export default function EngagementTrayPage() {
  const [trays, setTrays] = useState<TrayType[]>(staticTrays);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");
  const [loading, setLoading] = useState(true);

  // Fetch trays from backend
  useEffect(() => {
    const fetchTrays = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/engagementtrays");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
         const apiMapped: TrayType[] = data.map((item: any) => ({
          id: item._id || item.id,
        ...
        console.error("EngagementTray API fetch failed:", err.message);
  ...
  const categories = [...new Set(trays.map(i => i.category))];
  ...
  }, [trays, selectedCategories, highlight]);
