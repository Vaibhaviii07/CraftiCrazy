...
import { haldiPlatters as staticHaldiPlatters, HaldiPlatter as StaticHaldiPlatter } from "../../Data/HaldiPlatterData";
import { Link } from "react-router-dom";

export default function HaldiPlatterPage() {
  const [haldiPlatters, setHaldiPlatters] = useState<StaticHaldiPlatter[]>(staticHaldiPlatters);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");
  const [loading, setLoading] = useState(true);

  // Fetch Haldi Platters from backend
  useEffect(() => {
    const fetchPlatters = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/haldiplatters");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticHaldiPlatter[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setHaldiPlatters(apiMapped);
        }
      } catch (err: any) {
        console.error("HaldiPlatter API fetch failed:", err.message);
  ...
  }, [haldiPlatters, selectedCategories, highlight]);
