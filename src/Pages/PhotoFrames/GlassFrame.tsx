// src/Pages/GlassFrame/GlassFramePage.tsx
...
export default function GlassFramePage() {
  const [items, setItems] = useState<GlassFrame[]>(glassFrames); // fallback
  ...
  useEffect(() => {
    const fetchFrames = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/glassframes");
        ...
        console.error("Glass Frames API fetch failed:", err.message);
  ...
  const GlassFrameCard = ({ item }: { item: GlassFrame }) => {
    ...
    <section className="min-h-screen ">
      ...
      <div className="text-center mt-10 mb-8 px-4">
        ...
        <p className="mt-3 text-gray-600 text-base italic max-w-sm mx-auto">
          Elegant glass frames that highlight beauty and clarity.
        </p>
      </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
              <AnimatePresence>
                {sortedItems.map((item) => (
                  <GlassFrameCard key={item.id} item={item} />
                ))}
              </AnimatePresence>
            </div>
          )}
