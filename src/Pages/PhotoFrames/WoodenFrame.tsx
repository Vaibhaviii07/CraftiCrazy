// src/Pages/WoodenFrame/WoodenFramePage.tsx
...
export default function WoodenFramePage() {
  const [items, setItems] = useState<WoodenFrame[]>(woodenFrames); // fallback
  ...
  useEffect(() => {
    const fetchFrames = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/woodenframes");
        ...
        console.error("Wooden Frames API fetch failed:", err.message);
  ...
  const WoodenFrameCard = ({ item }: { item: WoodenFrame }) => {
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
              <AnimatePresence>
                {sortedItems.map((item) => (
                  <WoodenFrameCard key={item.id} item={item} />
                ))}
              </AnimatePresence>
            </div>
          )}
