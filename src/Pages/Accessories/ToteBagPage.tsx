// src/Pages/ToteBags/ToteBagPage.tsx
import { useState, useEffect, useMemo } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { toteBags as staticToteBags, ToteBag as StaticToteBag } from "../../Data/ToteBagData";
import { Link } from "react-router-dom";

export default function ToteBagPage() {
  const [toteBags, setToteBags] = useState<StaticToteBag[]>(staticToteBags);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");
  const [loading, setLoading] = useState(true);

  // Fetch Tote Bags from backend
  useEffect(() => {
    const fetchToteBags = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/totebags");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticToteBag[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setToteBags(apiMapped);
        } else {
          console.warn("API returned empty data, using static fallback.");
        }
      } catch (err: any) {
        console.error("ToteBag API fetch failed:", err.message);
