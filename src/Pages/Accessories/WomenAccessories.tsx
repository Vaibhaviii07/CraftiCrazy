...
import { womenAccessories as staticWomenAccessories, WomenAccessory as StaticWomenAccessory } from "../../Data/WomenAccessoriesData";

export default function WomenAccessoriesPage() {
  const [accessories, setAccessories] = useState<StaticWomenAccessory[]>(staticWomenAccessories);
  ...
  useEffect(() => {
    const fetchAccessories = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/womenaccessories");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticWomenAccessory[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setAccessories(apiMapped);
        }
      } catch (err: any) {
        console.error("WomenAccessories API fetch failed:", err.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const categories = [...new Set(accessories.map((i) => i.category))];
  ...
  }, [accessories, selectedCategories, highlight]);
