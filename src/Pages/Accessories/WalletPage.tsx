...
export default function WalletPage() {
  ...
  useEffect(() => {
    const fetchWallets = async () => {
      try {
        ...
        const apiMapped: Wallet[] = data.map((item: any) => ({
          id: item._id || item.id,
        ...
        setWalletsData(apiMapped);
        setDisplayedWallets(apiMapped);
      } catch (err) {
        console.error("Error fetching wallets, using static data:", err);
  ...
  useEffect(() => {
    ...
    let filtered = walletsData.filter((item) => {
      const categoryMatch =
  ...
  }, [selectedCategories, highlight, sortOption, walletsData]);
              ...
              {loading ? (
                <div className="col-span-full flex justify-center items-center py-20">
                  <motion.div
                    ...
                    className="w-12 h-12 border-4 border-t-[#C45A36] border-gray-200 rounded-full"
                  />
                </div>
              ) : (
