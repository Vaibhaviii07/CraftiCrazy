...
import { bracelets as staticBracelets, Bracelet as StaticBracelet } from "../../Data/BraceletData";
import { Link } from "react-router-dom";

export default function BraceletPage() {
  const [bracelets, setBracelets] = useState<StaticBracelet[]>(staticBracelets);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");
  const [loading, setLoading] = useState(true);
  // ✅ Fetch API data dynamically and merge/fallback
   useEffect(() => {
    const fetchBracelets = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/bracelets");
        if (!res.ok) throw new Error(`API fetch failed with status ${res.status}`);

        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticBracelet[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setBracelets(apiMapped);
        } else {
          console.warn("API returned no bracelet data — using static fallback.");
        }
      } catch (err: any) {
        console.error("Bracelet API fetch failed:", err.message);
  ...
  }, [bracelets, selectedCategories, highlight]);
