// src/Pages/KeyChains/KeyChainPage.tsx
import { useState, useMemo, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { keyChains as staticKeyChains, KeyChain as StaticKeyChain } from "../../Data/KeyChainData";
import { Link } from "react-router-dom";

export default function KeyChainPage() {
  const [keyChains, setKeyChains] = useState<StaticKeyChain[]>(staticKeyChains);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");
  const [loading, setLoading] = useState(true);

  // Fetch API data dynamically with fallback to static data
  useEffect(() => {
    const fetchKeyChains = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/keychains");
        if (!res.ok) throw new Error(`API fetch failed with status ${res.status}`);

        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: StaticKeyChain[] = data.map((item: any) => ({
            id: item._id || item.id,
          ...
          setKeyChains(apiMapped);
        } else {
          console.warn("API returned no keychain data — using static fallback.");
        }
      } catch (err: any) {
        console.error("KeyChain API fetch failed:", err.message);
  ...
  }, [keyChains, selectedCategories, highlight]);
    ...
    <section className="min-h-screen">
      ...
      <div className="text-center mt-10 mb-8">
        ...
        <p className="mt-3 text-gray-600 text-lg italic max-w-md mx-auto">
          Handcrafted with care and style, just for you.
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 mt-8 sm:mt-16 grid grid-cols-1 md:grid-cols-5 gap-6 md:gap-8">
        ...
        {/* Products Grid */}
        <div className="md:col-span-4 flex flex-col gap-6 mb-10">
          {/* Top Controls */}
          {!loading && (
