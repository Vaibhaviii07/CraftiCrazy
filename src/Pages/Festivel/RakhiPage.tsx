...
export default function RakhiPage() {
  const [items, setItems] = useState<RakhiKit[]>(rakhiKits); // fallback
  ...
  // Fetch API
  useEffect(() => {
    const fetchRakhis = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/rakhi");
        ...
        console.error("Rakhi API fetch failed:", err.message);
