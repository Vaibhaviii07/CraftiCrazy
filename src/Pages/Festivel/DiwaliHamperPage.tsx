...
export default function DiwaliHamperPage() {
  const [items, setItems] = useState<DiwaliHamper[]>(diwaliHampers);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/diwalihampers");
        ...
        console.error("Diwali Hampers API fetch failed:", err.message);
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
          ...
          {loading ? (
            <div className="flex justify-center items-center h-64">
              <div className="w-12 h-12 border-4 border-[#F7B77A] border-t-transparent rounded-full animate-spin"></div>
            </div>
          ) : (
