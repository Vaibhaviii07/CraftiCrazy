...
import { birthdayHampers as staticHampers, BirthdayHamper as StaticHamper } from "../../Data/BirthdayHampersdata";
import { Link } from "react-router-dom";

// ✅ TypeScript interface for Birthday Hamper
export interface BirthdayHamper {
  id: string;
  sku: string;
  name: string;
  description?: string;
...
export default function BirthdayHamperPage() {
  const [hampers, setHampers] = useState<BirthdayHamper[]>(staticHampers);
  const [selectedCategories, setSelectedCategories] = useState<string[]>([]);
  const [highlight, setHighlight] = useState("All");
  const [sortOption, setSortOption] = useState("Default sorting");
  const [loading, setLoading] = useState(true);

  // Fetch Birthday Hampers from API
  useEffect(() => {
    const fetchHampers = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/birthdayhampers");
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText}`);
        const data = await res.json();

        if (Array.isArray(data) && data.length > 0) {
          const apiMapped: BirthdayHamper[] = data.map((item: any) => ({
            id: item._id || item.id,
            ...
            variants: item.variants ?? [
              { image: item.image, price: item.price, discount: item.discount },
            ],
          }));
          setHampers(apiMapped);
        } else {
          console.warn("API returned empty data, using static fallback.");
        }
      } catch (err: any) {
        console.error("BirthdayHamper API fetch failed:", err.message);
  ...
  const categories = [...new Set(hampers.map((i) => i.category))];
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-12">
              <AnimatePresence>
                {sortedHampers.map((item) => {
                  const variant = item.variants?.[0] ?? { image: item.image, price: item.price, discount: item.discount };
                  return (
                    <motion.div
                      key={item.id}
                      ...
                      >
                        ...
                        <div className="mt-2 sm:mt-3 text-center px-1 sm:px-0">
                          ...
                          <div className="mt-1 sm:mt-2 flex justify-center gap-1 sm:gap-2 items-baseline">
                            <span className="text-lg sm:text-2xl text-[#C45A36] font-cinzel">₹{variant.price}</span>
                          </div>
                        </div>
                      </Link>
                    </motion.div>
