...
export default function CorporateHamper() {
  ...
  const handleAddToCart = (hamper: any) => {
    const exists = cart.find((c: any) => c.id === hamper.id);
    if (!exists) {
      addToCart({ ...hamper, quantity: 1 });
      setToast(`${hamper.name} added to cart!`);
    } else {
      setToast(`${hamper.name} is already in the cart`);
    }
  ...
  const filteredHampers = corporateHampers.filter((item) => {
    ...
    switch (highlight) {
      case "Top Rated":
        highlightMatch = Boolean(item.rating && item.rating > 4);
        break;
      case "Best Seller":
        highlightMatch = Boolean(item.popularity && item.popularity > 80);
        break;
      case "New Arrival":
        highlightMatch = Boolean(item.newArrival);
        break;
      default:
        highlightMatch = true;
    }
              ...
              {categories.map((cat) => (
                <li key={cat} className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    id={cat}
                    checked={Boolean(selectedCategories.includes(cat))}
